...
public class SpawnerVirus : MonoBehaviour
{
        ...
        void SpawnRandomEnemy()
    {
        ...
        if (spawnTimer >= spawnInteral)
        {
            ...
            if (temporal != null)
            {
                 ...
                 Vector3 posicionFija = spawnArea.bounds.center;
                // En Unity: almacena el resultado de la búsqueda en el NavMesh, el anterior
                temporal.transform.position = posicionFija;
    ...
    // void Start()
    // {
    //     InvokeRepeating("SpawnVirus", startDelay, repeatRate);
    // }

    // void SpawnVirus()
    // {
    //     Instantiate(enemyVirus, transform.position, transform.rotation);
    // }
}
